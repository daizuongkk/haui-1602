...
**Danh mục biến** khai báo tập trung tại `config.py: WEATHER_VARIABLES` (nhiệt độ, mưa, gió giật, CAPE, độ ẩm đất nhiều tầng, tầm nhìn, mực đông lạnh, bốc thoát hơi…). Toạ độ các huyện demo ở `locations.json`.
...
API mỏng **chỉ đọc**, hợp nhất `active_alerts.json` + `alert.json` + audio theo khoá `(địa điểm, ngày)`; suy giảm mềm khi thiếu bản dịch một ngày.

| Endpoint | Mô tả |
|---|---|
| `GET /api/locations` | Danh sách huyện + toạ độ |
| `GET /api/summary` | Mức cảnh báo cao nhất theo huyện + đếm KPI (tô màu bản đồ) |
| `GET /api/alerts/active` | Các ngày có cảnh báo (mức, hiểm họa, số liệu, bản tin, audio) |
| `GET /api/forecast/{location_id}` | Dự báo 3–7 ngày của một huyện |
| `POST /api/alerts/broadcast` | **Mô phỏng** nội dung gửi SMS / Zalo OA / loa (không gửi thật) |
| `/audio/...` | Phục vụ file MP3 tĩnh (UTF-8) |
...
> Ở bản demo, cả ba kênh SMS/Zalo/loa được **mô phỏng** qua `POST /api/alerts/broadcast` (dựng sẵn nội dung thật để trình diễn; tích hợp gửi thật là việc của giai đoạn triển khai — cần hợp đồng viễn thông/Zalo OA và webhook loa xã).
...
- **Chạy demo:**
  1. `python main.py` → sinh `active_alerts.json`
  2. `cd translation_agent && python main.py && python tts_reader.py` → bản tin + audio
...
| Yêu cầu | Đáp ứng |
|---|---|
| Demo dự báo 3–7 ngày cho ≥3 địa điểm | 3 huyện (Mường Nhé, Mường Chà, Tuần Giáo) × 7 ngày — `GET /api/forecast/{id}`, màn hình Người dân + Dashboard |
| Cơ chế cảnh báo tự động theo ngưỡng | `rule_engine.py` + `config.py: THRESHOLDS`, 7 nhóm hiểm họa 3 mức |
