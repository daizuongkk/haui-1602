...
## Stateful lifecycle layer (SQLite) — the "second half" of the flow

The JSON files above are the **read-only pipeline artifact**. On top of them sits a **stateful layer** (`data/app.db`, SQLAlchemy) that owns the alert **lifecycle** — this is what makes the full flow (approval → distribution → feedback) work:

- **Granularity is now commune (xã/cụm xã)**, not district. `data/communes.json` (generated by `application/communes.py` from the 3 districts) is the forecast unit; `forecast_pipeline` iterates it (falls back to `locations.json` if absent). Each commune has its own `real_elevation` → different downscaled alerts within one district.
- **DB layer:** `infrastructure/db/` (`models.py` = `Alert`/`Dispatch`/`Feedback`/`Officer`, `session.py`). `Alert` natural key is `UNIQUE(commune_id, date)` → re-running the pipeline **upserts**; content change after approval resets status to `pending_approval`.
- **Lifecycle states** live in `backend/shared/alert_status.py` (single source of truth, with a `TRANSITIONS` guard): `pending_approval → approved → distributed → closed` (+ `rejected`).
- **Use cases:** `application/{run_pipeline,alert_service,dispatch_service,feedback_service,seed}.py`. `run_pipeline` orchestrates forecast→translate→TTS→**upsert to DB**. Distribution goes through the `MessageChannel` port (`infrastructure/channels.py`, simulated `sent_sim`, swap for a real gateway later) and **persists a `Dispatch` row**.
- **API is now read+write** (not read-only): commune/officer/summary/dashboard reads, plus `POST /alerts/{id}/approve|reject|dispatch|feedback`, `PATCH .../status`, `POST /pipeline/run`. Officer identity via `X-Officer-Id` header (lightweight, no password; seeded from `settings.OFFICERS_SEED`). App startup (`api/app.py`) runs `init_db` + seeds officers + ensures communes.
- **Offline demo:** `cli seed` maps the existing 21 real bulletins onto communes as `pending_approval` alerts, so the whole workflow (and frontend) runs with no network/LLM. Live fresh data still needs `GEMINI_API_KEY` via `POST /pipeline/run`.
...
```bash
# Pipeline: fetch (per commune) → terrain-correct → evaluate → write data/active_alerts.json
python -m backend.presentation.cli            # (or: python main.py)
python -m backend.presentation.cli communes   # (re)generate data/communes.json (xã/cụm xã)
python -m backend.presentation.cli seed        # init SQLite + officers + demo alerts (offline, no LLM)
