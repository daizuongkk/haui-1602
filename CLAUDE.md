...
- Frontend: **Next.js 15 (App Router) + React 19 + TypeScript + Tailwind** (rebuilt on the `new-fe` branch — the old Vite/JS `src/` is gone). `next.config.ts` rewrites `/api/*` and `/audio/*` → FastAPI on `:8000` (set `BACKEND_URL` to change), so components use relative paths only. Google Maps needs `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` in `frontend/.env.local` (degrades to a placeholder if absent). Layering: `lib/` (`types` mirrors the API DTOs, `api` client, `levels` = the single source of truth for the 4-level scale, `i18n`, `hazards`), `components/{ui,common,citizen,dashboard}/`, `app/` routes (`/`, `/citizen`, `/dashboard{,/broadcast,/weather}`). **Real API only** — no risk-score/polygons/evacuation/timeline/email (backend has no data for them). Dev: `cd frontend && npm install && npm run dev` (port 5173).
